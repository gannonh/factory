...
The process binds to `127.0.0.1`. A command or a websocket upgrade whose `Origin` header is not the Factory page origin is rejected. There is no auth. The world is saved to disk as described in `docs/adr/0002-world-storage.md`. `sim.advance` stays on the in-process class for tests and is not a network command.
