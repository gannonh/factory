...
export function SandboxesView() {
  ...
  const leased = running.filter((s) => s.lease)
  ...
  return (
    <div className="absolute inset-0 overflow-y-auto p-4 flex flex-col gap-4">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <Kpi label="leased" value={`${leased.length}/${running.length || 0}`} sub="busy / running" color={leased.length ? '#22d3ee' : undefined} />
...
function CreateForm({ onDone }: { onDone: () => void }) {
  ...
  const submit = () => {
    if (!name.trim()) return
    api.sandboxes.create({ name: name.trim(), kind, host: host.trim() || HOST_HINT[kind], image: image.trim() })
    onDone()
  }
  return (
    <div className="rounded-xl border border-cyan-400/30 bg-ink-850 p-3 grid grid-cols-[1fr_120px_1fr_1fr_auto] gap-2 items-end">
