...
type Pasting = { paste: (fragment: GraphFragment, offset: Position) => GraphFragment }

export function createClipboard(history: Pasting, getWorld: () => World) {
  let held: { fragment: GraphFragment; pastes: number } | null = null

  const pasteAt = (fragment: GraphFragment, steps: number): NodeId[] =>
    history.paste(fragment, { x: PASTE_OFFSET * steps, y: PASTE_OFFSET * steps }).nodes.map((ref) => ref.node.id)
  ...
  return {
    ...
    duplicate: (ids: NodeId[]): NodeId[] => pasteAt(fragmentOf(getWorld(), ids), 1),
  }
}
